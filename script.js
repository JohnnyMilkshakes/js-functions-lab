...
console.log('Exercise 2 Result:', isAdult(25));
...
const greetUser = (name, time) => {
    if (time === 'morning') {
        ...
        return `Wen lunch ${name}?`
    } else if (time === 'evening') {
        return `Starting ${name}'s wind down time`
    } else {
        return `Bad news ${name}, you've either entered a time of day that isnt accounted for or you exist outside the limits of time, lets hope it's the former.`
    }
}
...
console.log('Exercise 6 Result:', maxOfThree(100, 300, 47));
...
const createUsername = (firstName, lastName) => {
    ...
    const len = concatNames.length
    
    return `${firstThree}${lastThree}${len}`
}
